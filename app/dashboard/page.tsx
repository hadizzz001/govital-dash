      ...
      <table className="table-auto w-full border-collapse border border-gray-200 mb-4">
        <thead>
          <tr className="bg-gray-100">
            ...
            <th className="border p-2">Sub brand</th>
        ...
        <tbody>
          {filteredProducts.map((product) => (
            <tr key={product.id} className="hover:bg-gray-50">
              ...
              <td className="border p-2">{product.subbrand}</td>
...
function EditProductForm({ product, onCancel, onSave }) {
  ...
  const [selectedSubbrand, setSelectedSubbrand] = useState(product.subbrand || "");

  const [isEditingCategory, setIsEditingCategory] = useState(false);
  const [isEditingBrand, setIsEditingBrand] = useState(false);
  const [isEditingSubbrand, setIsEditingSubbrand] = useState(false);

  // Fetch options for category, brand, and subbrand
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        const [categoriesRes, brandsRes, subbrandsRes] = await Promise.all([
          fetch("/api/category"),
          fetch("/api/brand"),
          fetch("/api/subbrand"),
        ]);
        ...
        setSubbrands(await subbrandsRes.json());
      } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="border p-4 bg-gray-100 rounded">
      ...
      {/* Subbrand Input/Select */}
      <div className="mb-4">
        <label htmlFor="subbrand" className="block text-sm font-medium text-gray-700">
          Subbrand
        </label>
        {isEditingSubbrand ? (
          <select
            ...
            onBlur={() => setIsEditingSubbrand(false)}
            className="w-full border p-2"
          >
            <option value="">Select Subbrand</option>
            {subbrands.map((subbrand) => (
              <option key={subbrand.id} value={subbrand.name}>
                {subbrand.name}
              </option>
            ))}
          </select>
          ...
          <input
            type="text"
            value={selectedSubbrand}
            onClick={() => setIsEditingSubbrand(true)}
            readOnly
            className="w-full border p-2 cursor-pointer"
          />
        )}
